...
<p align="center">
  ...
  <a href="https://codecov.io/gh/dantebertuzzi/Perth.jl"><img alt="codecov" src="https://codecov.io/gh/dantebertuzzi/Perth.jl/branch/main/graph/badge.svg"></a>
...
| **资源平衡** | `level!` — 推迟有浮时的任务，直到没有人超出所声明的每日产能；浮时最少者优先保留 |
...
- 按人或团队分的**泳道**、可折叠的 **WBS 摘要**（折叠状态还能挺过刷新）、**高亮筛选**
  ——不匹配的会变暗，计划长到"划过灰色就是大半阅读量"时也可以直接隐藏（*只看这些*、
  `O`）——和**演示模式**。
...
| | |
|---|---|
| **S 曲线** | 计划 × 实际，并直接说出数字：*低于至今的计划 13%*，工作量和金额各一条 |
| **负荷** | 每个人每天有多少活（`workload`、`overallocations`） |
| **产能** | `add_person!(p, "Ana"; capacity = 8)` 加上任务的 `effort`：超载从"同一天两个任务"变成"超过这天容得下的工作量" |
...
**也能走回来。** CSV 同样可以导入 —— *文件 → 导入*，或在 REPL 里
`add_tasks!(p, "plan.csv")`，不需要安装任何 CSV 包。每行一个任务，一列 `name`，其余
列有什么算什么；`parent` 和 `dependencies` 可以用任务**名称**来指代，所以别人真正写出
来的表格，和 Perth 导出的文件一样能进得来。
...
按列的 **WIP 上限**和归档；列可以随时按截止日期排序，也可以按创建时间排序、最新的卡片排在最上面 ——
前者是紧急程度，后者是刚刚进来的东西。把关联卡片拖到 *done* 会完成甘特图里的对应任务，反之亦然。
卡片还能**作为文档打开**（`Shift+Enter`，或从卡片自己的编辑器进入 —— 平板没有
Shift 键）：带列表和代码块的描述，以及用
`Ctrl+V` 直接粘贴的截图 —— 在浏览器里先缩小，存放在看板旁边并按内容寻址，所以同一张
图片粘贴五次也只是一个文件。
`Ctrl+Z` / `Ctrl+Shift+Z` 撤销**你自己**的操作，而不会回退同事之后做的改动。

**主机**可以限制某台机器能做什么 —— *Board → Permissions…* 是一张矩阵：21 项卡片与
...
| `N` · `Enter` · `Del` · `Ctrl+D` | 新建 · 编辑 · 删除 · 复制所选 |
| `Ctrl+点击` · `Shift+点击` · `Shift+↑` `↓` | 加入选择 · 选中区间 · 扩展选择 |
| `Ctrl+A` · `Ctrl+E` | 选中所有可见任务 · 编辑所选（日期、负责人、颜色） |
...
- **资源平衡是启发式的，并且需要声明产能。** `level!` 先移动浮时最多的任务——这是一条站得住脚的
  规则，而不是最优解，因为最优解是 NP 难问题。它只处理声明了 `capacity` 的人；工作量超过产能的
  计划返回时，仍然放不下的天数会被标记出来，而不是被悄悄重排。
