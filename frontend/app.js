...
async function loadProjects(keepId = null) {
  ...
  renderProjectSelect();
  const wanted = keepId ?? state.current?.id ?? state.projects[0]?.id ?? null;
...
function openModal(id) {
  ...
  for (const o of others) {
    ...
    cb.type = "checkbox";
    cb.value = o.id;
    cb.checked = (t.dependencies || []).includes(o.id);
...
function submitModal() {
  ...
  t.dependencies = $$("#f-deps input:checked").map((cb) => cb.value);
...
function exportProject() {
  ...
  a.href = `/api/projects/${state.current.id}/export`;
  a.download = "";
  a.click();
}
